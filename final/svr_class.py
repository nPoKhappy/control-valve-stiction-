...
class Svr():
    ...
    def draw_validation_mse(self, show: bool)->None:
        ...
        """MSE plot"""
        plt.figure()
        plt.plot(range(len(valid_mean_squared_error)), valid_mean_squared_error)
        plt.xlabel("$C$ $and$ $epsilon$ $pair$ $coefficient$")
        plt.ylabel("$MSE$")
        plt.title("MSE varies with C and epsilon pair")
        plt.show()
